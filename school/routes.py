...
#Le route de la page d'acceuil ou tous les articles sont postés par soit le blogger ou l'admin(demba sarr)
@app.route("/")
@app.route("/home")
def home():
...
#Permet aux deux utilisateurs(pour l'instant le blogger et demba sarr) de se connecter 
...
#Modifier le mot de passe apres la demande
...
def update_classe(classe_id):
    classe = Classe.query.get_or_404(classe_id)
    if current_user.username != 'Demba Sarr':
        abort(403)
    form = ClasseForm()
    if form.validate_on_submit():
